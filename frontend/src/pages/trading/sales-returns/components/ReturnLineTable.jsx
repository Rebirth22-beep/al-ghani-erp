...
export function ReturnLineTable({ control, register, errors, setValue, watch }) {
  const { fields, append, remove } = useFieldArray({ control, name: 'lines' })

  const addLine = () =>
    append({ product: null, product_id: '', return_qty: 1, unit: '', rate_rupees: 0, rate_paisas: 0, total_paisas: 0 })

  const recalcTotal = (index) => {
    const qty  = parseFloat(watch(`lines.${index}.return_qty`) || 0)
    const rate = parseFloat(watch(`lines.${index}.rate_rupees`) || 0)
    const total = toPaisas(qty * rate)
    setValue(`lines.${index}.rate_paisas`,  toPaisas(rate))
    ...
    <div className="space-y-2">
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="w-full text-sm">
          <thead>
            <tr>
              {['Product', 'Return Qty', 'Unit', 'Rate (Rs)', 'Total (Rs)', ''].map((h) => (
              ...
              <tr key={field.id}>
                    ...
                    render={({ field: f }) => (
                      <SearchProductInput
                        ...
                        onChange={(p) => {
                          ...
                          setValue(`lines.${index}.rate_rupees`, toRupees(p.sale_rate_paisas || 0))
                          recalcTotal(index)
                        }}
                      />
                    )}
                ...
                <td className="table-td w-24">
                  <input
                    ...
                    min="0.001"
                    step="0.001"
                    {...register(`lines.${index}.return_qty`)}
                ...
                <td className="table-td w-28">
                  <input
                    ...
                    className="form-input"
                    step="0.01"
                    {...register(`lines.${index}.rate_rupees`)}
                    onBlur={() => recalcTotal(index)}
                  />
                </td>
