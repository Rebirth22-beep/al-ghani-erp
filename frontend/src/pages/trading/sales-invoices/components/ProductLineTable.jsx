...
export function ProductLineTable({ control, register, errors, setValue, watch }) {
  const { fields, append, remove } = useFieldArray({ control, name: 'lines' })

  const addLine = () =>
    append({ product: null, product_id: '', quantity: 1, unit: '', rate_paisas: 0, total_paisas: 0, batch_id: '' })
    ...
    <div className="space-y-2">
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="w-full text-sm">
          <thead>
            <tr>
              {['Product', 'Qty', 'Unit', 'Rate (Rs)', 'Total (Rs)', ''].map((h) => (
                    ...
                    render={({ field: f }) => (
                      <SearchProductInput
                        ...
                        onChange={(p) => {
                          ...
                          setValue(`lines.${index}.rate_rupees`,  toRupees(p.sale_rate_paisas))
                          recalcTotal(index)
                        }}
                      />
                    )}
