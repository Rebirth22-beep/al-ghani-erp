...
export function CustomerTable({ data, loading, onDelete }) {
  const columns = [
    { key: 'name',            title: 'Name' },
    ...
    { key: 'balance_paisas',  title: 'Balance', render: (v) => <span className={`font-medium ${v < 0 ? 'text-red-600' : 'text-green-700'}`}>{formatCurrency(v)}</span> },
    {
      ...
      render: (id) => (
        <div className="flex items-center gap-2">
          <Link to={`/parties/customers/${id}/edit`} className="text-xs text-brand hover:underline">Edit</Link>
          <Link to={`/parties/customers/${id}/ledger`} className="text-xs text-indigo-600 hover:underline">Ledger</Link>
